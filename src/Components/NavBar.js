        ...
        return(
            <nav className="navbar navbar-expand-lg navbar-dark bg-dark flex-row navbar-inverse text-center">
                ...
                <form className="form-inline form-group d-inline justify-content-center mx-auto my-2 my-lg-0">
                        <input className="form-control form-control-lg search mr-sm-2" type="search" placeholder="Type to begin" aria-label="Search" value={this.props.searchText} onChange={(e) => this.props.handleInput(e)} />
                ...
                <ul className="navbar-nav">
                    ...
                    <img className="nav-avatar" src={this.props.user ? this.props.user.photoURL : require('../img/user.png')} alt="avatar"/>{this.props.user ? this.props.user.displayName : "Account" }</Link>
                    </li>
                </ul>
                {this.props.user ? 
